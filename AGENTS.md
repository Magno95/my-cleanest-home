...
Web app to manage household cleaning: map rooms/areas/items, set cleaning frequencies, view a calendar of due tasks, mark tasks done to restart their cycle, and look up how to clean a given surface via linked tools/products. Multi-home, multi-user with task assignment. A separate admin web app ("CMS") manages products, instructions, and config centrally.

## Stack (confirmed)

- **Monorepo**: pnpm workspaces + Turborepo
- **Frontend (`apps/web`, `apps/cms`)**: React + Vite + TypeScript
...
- **Shared code**: `packages/shared` (types, domain logic), `packages/ui` (React components shared across `web` and `cms`)
...
apps/
  ...
  cms/     # admin React app (web-based, not Electron)
  api/     # Express API
packages/
...
- Domain types (Home, Room, Area, Item, Task, Frequency, CleaningCycle, Product, Tool, User, Assignment) belong in `packages/shared` and are imported by both apps and api.
...
- Electron / native desktop packaging — CMS is a web app.
- Alternative ORMs (Prisma/Drizzle) — data access goes through Supabase clients.
- Extra apps beyond `web`, `cms`, `api`.
